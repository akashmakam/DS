...
// Define a queue structure to manage the priority queue
typedef struct q {
...
void enqueue(q *q) {
    ...
    // If queue is empty or the new data has higher priority, insert at front
    if (q -> front == NULL || q -> front -> data > newNode -> data) {
        newNode -> next = q -> front; 
        q -> front = newNode;
    } else {
        // Otherwise, find the appropriate position and insert
        node *current = q -> front;
